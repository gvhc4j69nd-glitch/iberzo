...
export default function QwirkleTile({ shape, color, size = 52, selected, staged, className = '', onClick, style = {} }) {
  const fill = COLOR_VALUES[color] || '#888';
  const ShapeComp = SHAPE_COMPONENTS[shape];

  return (
    <div
      className={`qwerkle-tile ${selected ? 'selected' : ''} ${staged ? 'staged' : ''} ${className}`}
