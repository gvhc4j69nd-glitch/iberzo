...
const HAND_SIZE = 6;
const QWERKLE_SIZE = 6;
const QWERKLE_BONUS = 6;
...
function isValidLine(board, positions) {
  if (positions.length > QWERKLE_SIZE) return false;
...
function scoreMove(board, placements) {
  ...
  for (const { x, y } of placements) {
    for (const dir of ['h', 'v']) {
      ...
      total += line.length;
      if (line.length === QWERKLE_SIZE) total += QWERKLE_BONUS;
    }
  }
...
function applyMove(state, userId, placements) {
  ...
  if (newHand.length === 0 && newBag.length === 0) {
    status = 'finished';
    newPlayers[playerIdx] = { ...newPlayers[playerIdx], score: newPlayers[playerIdx].score + QWERKLE_BONUS };
  }
